...
--------------------------------------------------------------------------------
T_5d = {}
for i = 1, NElectrons_5d / 2 do
    T_5d[2*i - 1] = NewOperator('An', NFermions, IndexDn_5d[i])
    T_5d[2*i]     = NewOperator('An', NFermions, IndexUp_5d[i])
end
...
if ValueInTable(spectrum, spectra) then
    indices_5d[spectrum] = {}
    for j, operator in ipairs(T_5d) do
        table.insert(indices_5d[spectrum], c)
        c = c + 1
    end
end
...
if CalculationRestrictions == nil then
    G_5d = CreateSpectra(H_f, T_5d, Psis_i, {{'Emin', Emin}, {'Emax', Emax}, {'NE', NE}, {'Gamma', Gamma}, {'DenseBorder', DenseBorder}})
else
    G_5d = CreateSpectra(H_f, T_5d, Psis_i, {{'Emin', Emin}, {'Emax', Emax}, {'NE', NE}, {'Gamma', Gamma}, {'restrictions', CalculationRestrictions}, {'DenseBorder', DenseBorder}})
end

-- Create a list with the Boltzmann probabilities for a given operator
-- and state.
dZ_5d = {}
for i in ipairs(T_5d) do
    for j in ipairs(Psis_i) do
        table.insert(dZ_5d, dZ[j])
    end
end

spectrum = 'Isotropic'
if ValueInTable(spectrum, spectra) then
    Giso = GetSpectrum(G_5d, T_5d, Psis_i, indices_5d[spectrum], dZ_5d)
    SaveSpectrum(Giso / #T_5d, 'iso')
end

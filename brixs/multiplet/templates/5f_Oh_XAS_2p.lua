...
function CalculateT(e, k)
    ...
    T = (e[1] * k[2] + e[2] * k[1]) * Txy_2p_5f
      + (e[1] * k[3] + e[3] * k[1]) * Txz_2p_5f
      + (e[2] * k[3] + e[3] * k[2]) * Tyz_2p_5f
      + (e[1] * k[1] + e[2] * k[2]) * Tx2y2_2p_5f
      + e[3] * k[3] * Tz2_2p_5f
    return Chop(T)
end

Tv_2p_5f = CalculateT(ev, k)
Th_2p_5f = CalculateT(eh, k)
Tr_2p_5f = CalculateT(er, k)
Tl_2p_5f = CalculateT(el, k)
Tk_2p_5f = CalculateT(k, k)
...
if ValueInTable(spectrum, spectra) then
    indices_2p_5f[spectrum] = {}
    for j, operator in ipairs({Txy_2p_5f, Txz_2p_5f, Tyz_2p_5f, Tx2y2_2p_5f, Tz2_2p_5f}) do
        table.insert(T_2p_5f, operator)
        table.insert(indices_2p_5f[spectrum], c)
        c = c + 1
    end
end
...
if ValueInTable(spectrum, spectra) then
    indices_2p_5f[spectrum] = {}
    for j, operator in ipairs({Tr_2p_5f, Tl_2p_5f}) do
        table.insert(T_2p_5f, operator)
        table.insert(indices_2p_5f[spectrum], c)
        c = c + 1
    end
...
if ValueInTable(spectrum, spectra) then
    indices_2p_5f[spectrum] = {}
    for j, operator in ipairs({Tv_2p_5f, Th_2p_5f}) do
        table.insert(T_2p_5f, operator)
        table.insert(indices_2p_5f[spectrum], c)
        c = c + 1
    end
...
if CalculationRestrictions == nil then
    G_2p_5f = CreateSpectra(H_f, T_2p_5f, Psis_i, {{'Emin', Emin}, {'Emax', Emax}, {'NE', NE}, {'Gamma', Gamma}, {'DenseBorder', DenseBorder}})
else
    G_2p_5f = CreateSpectra(H_f, T_2p_5f, Psis_i, {{'Emin', Emin}, {'Emax', Emax}, {'NE', NE}, {'Gamma', Gamma}, {'restrictions', CalculationRestrictions}, {'DenseBorder', DenseBorder}})
end

-- Create a list with the Boltzmann probabilities for a given operator
-- and state.
dZ_2p_5f = {}
for i in ipairs(T_2p_5f) do
    for j in ipairs(Psis_i) do
        table.insert(dZ_2p_5f, dZ[j])
    end
end
...
spectrum = 'Isotropic'
if ValueInTable(spectrum, spectra) then
    Giso = GetSpectrum(G_2p_5f, T_2p_5f, Psis_i, indices_2p_5f[spectrum], dZ_2p_5f)
    Giso = Giso / 15 / Pcl_2p_5f
    SaveSpectrum(Giso, 'iso')
end

spectrum = 'Circular Dichroism'
if ValueInTable(spectrum, spectra) then
    Gr = GetSpectrum(G_2p_5f, T_2p_5f, Psis_i, indices_2p_5f[spectrum][1], dZ_2p_5f)
    Gl = GetSpectrum(G_2p_5f, T_2p_5f, Psis_i, indices_2p_5f[spectrum][2], dZ_2p_5f)
    Gr = Gr / Pcl_2p_5f
    Gl = Gl / Pcl_2p_5f
    SaveSpectrum(Gr, 'r')
    SaveSpectrum(Gl, 'l')
...
spectrum = 'Linear Dichroism'
if ValueInTable(spectrum, spectra) then
    Gv = GetSpectrum(G_2p_5f, T_2p_5f, Psis_i, indices_2p_5f[spectrum][1], dZ_2p_5f)
    Gh = GetSpectrum(G_2p_5f, T_2p_5f, Psis_i, indices_2p_5f[spectrum][2], dZ_2p_5f)
    Gv = Gv / Pcl_2p_5f
    Gh = Gh / Pcl_2p_5f
    SaveSpectrum(Gv, 'v')
    SaveSpectrum(Gh, 'h')

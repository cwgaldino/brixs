...
function CalculateT(e)
    ...
    T = e[1] * Tx_4d_4f + e[2] * Ty_4d_4f + e[3] * Tz_4d_4f
    return Chop(T)
end

Tv_4d_4f = CalculateT(ev)
Th_4d_4f = CalculateT(eh)
Tr_4d_4f = CalculateT(er)
Tl_4d_4f = CalculateT(el)
Tk_4d_4f = CalculateT(k)
...
if ValueInTable(spectrum, spectra) then
    indices_4d_4f[spectrum] = {}
    for j, operator in ipairs({Tr_4d_4f, Tl_4d_4f, Tk_4d_4f}) do
        table.insert(T_4d_4f, operator)
        table.insert(indices_4d_4f[spectrum], c)
        c = c + 1
    end
end
...
if ValueInTable(spectrum, spectra) then
    ...
    if ValueInTable('Isotropic', spectra) then
        table.insert(indices_4d_4f[spectrum], 1)
        table.insert(indices_4d_4f[spectrum], 2)
    else
        for j, operator in ipairs({Tr_4d_4f, Tl_4d_4f}) do
            table.insert(T_4d_4f, operator)
            table.insert(indices_4d_4f[spectrum], c)
            c = c + 1
        end
...
if ValueInTable(spectrum, spectra) then
    indices_4d_4f[spectrum] = {}
    for j, operator in ipairs({Tv_4d_4f, Th_4d_4f}) do
        table.insert(T_4d_4f, operator)
        table.insert(indices_4d_4f[spectrum], c)
        c = c + 1
    end
...
if CalculationRestrictions == nil then
    G_4d_4f = CreateSpectra(H_f, T_4d_4f, Psis_i, {{'Emin', Emin}, {'Emax', Emax}, {'NE', NE}, {'Gamma', Gamma}, {'DenseBorder', DenseBorder}})
else
    G_4d_4f = CreateSpectra(H_f, T_4d_4f, Psis_i, {{'Emin', Emin}, {'Emax', Emax}, {'NE', NE}, {'Gamma', Gamma}, {'restrictions', CalculationRestrictions}, {'DenseBorder', DenseBorder}})
end

-- Create a list with the Boltzmann probabilities for a given operator
-- and state.
dZ_4d_4f = {}
for i in ipairs(T_4d_4f) do
    for j in ipairs(Psis_i) do
        table.insert(dZ_4d_4f, dZ[j])
    end
end
...
spectrum = 'Isotropic'
if ValueInTable(spectrum, spectra) then
    Giso = GetSpectrum(G_4d_4f, T_4d_4f, Psis_i, indices_4d_4f[spectrum], dZ_4d_4f)
    Giso = Giso / 3 / Pcl_4d_4f
    SaveSpectrum(Giso, 'iso')
end

spectrum = 'Circular Dichroism'
if ValueInTable(spectrum, spectra) then
    Gr = GetSpectrum(G_4d_4f, T_4d_4f, Psis_i, indices_4d_4f[spectrum][1], dZ_4d_4f)
    Gl = GetSpectrum(G_4d_4f, T_4d_4f, Psis_i, indices_4d_4f[spectrum][2], dZ_4d_4f)
    Gr = Gr / Pcl_4d_4f
    Gl = Gl / Pcl_4d_4f
    SaveSpectrum(Gr, 'r')
    SaveSpectrum(Gl, 'l')
...
spectrum = 'Linear Dichroism'
if ValueInTable(spectrum, spectra) then
    Gv = GetSpectrum(G_4d_4f, T_4d_4f, Psis_i, indices_4d_4f[spectrum][1], dZ_4d_4f)
    Gh = GetSpectrum(G_4d_4f, T_4d_4f, Psis_i, indices_4d_4f[spectrum][2], dZ_4d_4f)
    Gv = Gv / Pcl_4d_4f
    Gh = Gh / Pcl_4d_4f
    SaveSpectrum(Gv, 'v')
    SaveSpectrum(Gh, 'h')

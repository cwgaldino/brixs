...
function CalculateT(e)
    ...
    T = e[1] * Tx_3p_3d + e[2] * Ty_3p_3d + e[3] * Tz_3p_3d
    return Chop(T)
end

Tv_3p_3d = CalculateT(ev)
Th_3p_3d = CalculateT(eh)
Tr_3p_3d = CalculateT(er)
Tl_3p_3d = CalculateT(el)
Tk_3p_3d = CalculateT(k)
...
if ValueInTable(spectrum, spectra) then
    ...
    for j, operator in ipairs({Tr_3p_3d, Tl_3p_3d, Tk_3p_3d}) do
        table.insert(T_3p_3d, operator)
        table.insert(indices_3p_3d[spectrum], c)
        c = c + 1
    end
end
...
if ValueInTable(spectrum, spectra) then
    ...
    if ValueInTable('Isotropic', spectra) then
        ...
        for j, operator in ipairs({Tr_3p_3d, Tl_3p_3d}) do
            table.insert(T_3p_3d, operator)
            table.insert(indices_3p_3d[spectrum], c)
            c = c + 1
        end
...
if ValueInTable(spectrum, spectra) then
    indices_3p_3d[spectrum] = {}
    for j, operator in ipairs({Tv_3p_3d, Th_3p_3d}) do
        table.insert(T_3p_3d, operator)
        table.insert(indices_3p_3d[spectrum], c)
        c = c + 1
    end
...
if CalculationRestrictions == nil then
    G_3p_3d = CreateSpectra(H_f, T_3p_3d, Psis_i, {{'Emin', Emin}, {'Emax', Emax}, {'NE', NE}, {'Gamma', Gamma}, {'DenseBorder', DenseBorder}})
else
    G_3p_3d = CreateSpectra(H_f, T_3p_3d, Psis_i, {{'Emin', Emin}, {'Emax', Emax}, {'NE', NE}, {'Gamma', Gamma}, {'restrictions', CalculationRestrictions}, {'DenseBorder', DenseBorder}})
end

-- Create a list with the Boltzmann probabilities for a given operator
-- and state.
dZ_3p_3d = {}
for i in ipairs(T_3p_3d) do
    for j in ipairs(Psis_i) do
        table.insert(dZ_3p_3d, dZ[j])
    end
end

...
if H_atomic == 1 then
    ...
    F2_3p_4f_f = $F2(3p,4f)_f_value * $F2(3p,4f)_f_scale
    G2_3p_4f_f = $G2(3p,4f)_f_value * $G2(3p,4f)_f_scale
    G4_3p_4f_f = $G4(3p,4f)_f_value * $G4(3p,4f)_f_scale
...
Tv_3p_4f = CalculateT(ev, k)
Th_3p_4f = CalculateT(eh, k)
Tr_3p_4f = CalculateT(er, k)
Tl_3p_4f = CalculateT(el, k)
Tk_3p_4f = CalculateT(k, k)
...
if ValueInTable(spectrum, spectra) then
    indices_3p_4f[spectrum] = {}
    for j, operator in ipairs({Txy_3p_4f, Txz_3p_4f, Tyz_3p_4f, Tx2y2_3p_4f, Tz2_3p_4f}) do
        table.insert(T_3p_4f, operator)
        table.insert(indices_3p_4f[spectrum], c)
        c = c + 1
    end
end
...
if ValueInTable(spectrum, spectra) then
    indices_3p_4f[spectrum] = {}
    for j, operator in ipairs({Tr_3p_4f, Tl_3p_4f}) do
        table.insert(T_3p_4f, operator)
        table.insert(indices_3p_4f[spectrum], c)
        c = c + 1
    end
...
if ValueInTable(spectrum, spectra) then
    indices_3p_4f[spectrum] = {}
    for j, operator in ipairs({Tv_3p_4f, Th_3p_4f}) do
        table.insert(T_3p_4f, operator)
        table.insert(indices_3p_4f[spectrum], c)
        c = c + 1
    end
...
if CalculationRestrictions == nil then
    G_3p_4f = CreateSpectra(H_f, T_3p_4f, Psis_i, {{'Emin', Emin}, {'Emax', Emax}, {'NE', NE}, {'Gamma', Gamma}, {'DenseBorder', DenseBorder}})
else
    G_3p_4f = CreateSpectra(H_f, T_3p_4f, Psis_i, {{'Emin', Emin}, {'Emax', Emax}, {'NE', NE}, {'Gamma', Gamma}, {'restrictions', CalculationRestrictions}, {'DenseBorder', DenseBorder}})
end

-- Create a list with the Boltzmann probabilities for a given operator
-- and state.
dZ_3p_4f = {}
for i in ipairs(T_3p_4f) do
    for j in ipairs(Psis_i) do
        table.insert(dZ_3p_4f, dZ[j])
    end
end
...
spectrum = 'Isotropic'
if ValueInTable(spectrum, spectra) then
    Giso = GetSpectrum(G_3p_4f, T_3p_4f, Psis_i, indices_3p_4f[spectrum], dZ_3p_4f)
    Giso = Giso / 15 / Pcl_3p_4f
    SaveSpectrum(Giso, 'iso')
end

spectrum = 'Circular Dichroism'
if ValueInTable(spectrum, spectra) then
    Gr = GetSpectrum(G_3p_4f, T_3p_4f, Psis_i, indices_3p_4f[spectrum][1], dZ_3p_4f)
    Gl = GetSpectrum(G_3p_4f, T_3p_4f, Psis_i, indices_3p_4f[spectrum][2], dZ_3p_4f)
    Gr = Gr / Pcl_3p_4f
    Gl = Gl / Pcl_3p_4f
    SaveSpectrum(Gr, 'r')
    SaveSpectrum(Gl, 'l')
...
spectrum = 'Linear Dichroism'
if ValueInTable(spectrum, spectra) then
    Gv = GetSpectrum(G_3p_4f, T_3p_4f, Psis_i, indices_3p_4f[spectrum][1], dZ_3p_4f)
    Gh = GetSpectrum(G_3p_4f, T_3p_4f, Psis_i, indices_3p_4f[spectrum][2], dZ_3p_4f)
    Gv = Gv / Pcl_3p_4f
    Gh = Gh / Pcl_3p_4f
    SaveSpectrum(Gv, 'v')
    SaveSpectrum(Gh, 'h')

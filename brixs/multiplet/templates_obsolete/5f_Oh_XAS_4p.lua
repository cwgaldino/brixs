...
if H_atomic == 1 then
    ...
    F2_4p_5f_f = $F2(4p,5f)_f_value * $F2(4p,5f)_f_scale
    G2_4p_5f_f = $G2(4p,5f)_f_value * $G2(4p,5f)_f_scale
    G4_4p_5f_f = $G4(4p,5f)_f_value * $G4(4p,5f)_f_scale
...
function CalculateT(e, k)
    ...
    T = (e[1] * k[2] + e[2] * k[1]) * Txy_4p_5f
      + (e[1] * k[3] + e[3] * k[1]) * Txz_4p_5f
      + (e[2] * k[3] + e[3] * k[2]) * Tyz_4p_5f
      + (e[1] * k[1] + e[2] * k[2]) * Tx2y2_4p_5f
      + e[3] * k[3] * Tz2_4p_5f
    return Chop(T)
end

Tv_4p_5f = CalculateT(ev, k)
Th_4p_5f = CalculateT(eh, k)
Tr_4p_5f = CalculateT(er, k)
Tl_4p_5f = CalculateT(el, k)
Tk_4p_5f = CalculateT(k, k)
...
if ValueInTable(spectrum, spectra) then
    indices_4p_5f[spectrum] = {}
    for j, operator in ipairs({Txy_4p_5f, Txz_4p_5f, Tyz_4p_5f, Tx2y2_4p_5f, Tz2_4p_5f}) do
        table.insert(T_4p_5f, operator)
        table.insert(indices_4p_5f[spectrum], c)
        c = c + 1
    end
end
...
if ValueInTable(spectrum, spectra) then
    indices_4p_5f[spectrum] = {}
    for j, operator in ipairs({Tr_4p_5f, Tl_4p_5f}) do
        table.insert(T_4p_5f, operator)
        table.insert(indices_4p_5f[spectrum], c)
        c = c + 1
    end
...
if ValueInTable(spectrum, spectra) then
    indices_4p_5f[spectrum] = {}
    for j, operator in ipairs({Tv_4p_5f, Th_4p_5f}) do
        table.insert(T_4p_5f, operator)
        table.insert(indices_4p_5f[spectrum], c)
        c = c + 1
    end
...
if CalculationRestrictions == nil then
    G_4p_5f = CreateSpectra(H_f, T_4p_5f, Psis_i, {{'Emin', Emin}, {'Emax', Emax}, {'NE', NE}, {'Gamma', Gamma}, {'DenseBorder', DenseBorder}})
else
    G_4p_5f = CreateSpectra(H_f, T_4p_5f, Psis_i, {{'Emin', Emin}, {'Emax', Emax}, {'NE', NE}, {'Gamma', Gamma}, {'restrictions', CalculationRestrictions}, {'DenseBorder', DenseBorder}})
end

-- Create a list with the Boltzmann probabilities for a given operator
-- and state.
dZ_4p_5f = {}
for i in ipairs(T_4p_5f) do
    for j in ipairs(Psis_i) do
        table.insert(dZ_4p_5f, dZ[j])
    end
end

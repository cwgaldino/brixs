...
class Peak(lmfit.Parameters):
    ...
    def _find_suitable_x(self):
        ...
        vmin = self['c'].value-5*w
        vmax = self['c'].value+5*w
...
class Peaks(MutableMapping):
    ...
    def generate_residual_function(self, x, y, yerr=None):
    ...
    def fit(self, x, y, yerr=None, method='least_squares', return_fitted_peaks=False):
        """output, peaks = fit()
        
        I think x and y must be monotonic.
        """
        residual = self.generate_residual_function(x=x, y=y, yerr=yerr)
        if yerr is None:
            out = lmfit.minimize(residual, method=method, params=self._add_peaks(), args=(x, y))
        else:
            out = lmfit.minimize(residual, method=method, params=self._add_peaks(), args=(x, y, yerr))
    ...
    def copy(self, value=None):
        """if value is None, returns a copy.
            if value is of type Peaks(), copy the peaks to this object
        """
        if value is None:
            temp = Peaks()
            for peak in self:
            ...
            return temp
        elif isinstance(value, Peaks):
            self.clear()
            for peak in value:
                self.append(peak.copy())
        else:

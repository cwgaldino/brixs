...
# Peaks ========================================================================
class Additional(metaclass=br._Meta):
    """
    """
    _read_only     = []
    _non_removable = []
...
class Peaks(lmfit.Parameters):
    ...
    valid attrs are: 'amp', 'area', 'c', 'w', 'w1', 'w2', 'm', 'm1', 'm2'
    which are the parameters os a pseudo-voigt curve

    ########## Additional types of curves can be defined. I will teach how to do that soon. TODO

    ########## talk about pseudo attrs

    # peaks can be accessed via indexing 
    peaks[i2]     -> return all peaks for spectrum i2
    peaks[i2][i1] -> return peak i1 of spectrum i2
    if peaks only has data for one spectrum (unique i2)
    peaks[i1]     -> return peak i1 for the only unique i2

    negative i1 and i2 runs backwards
    ...
    def is_asymmetric(self, i1=None, i2=None):
        ...
        Args:
            ...
            i2 (int, optional): secondary (spectrum) index i2. Default is None.
        
        Returns:
            bool
        """
        i1, i2 = self._check_i1_i2(i1=i1, i2=i2, i1_req=True, i2_req=True)
        ...
        if f'w1_{i1}_{i2}' in self and f'w2_{i1}_{i2}' in self:
            ...
            raise ValueError('peak does not have w parameter defined')
    
    def use_area(self, i1=None, i2=None):
    ...
    # def _get_additional_indexes_i1(self, i2=None):
    #     """return list of all primary indexes i1's for additional parameters
    ...
    # def add(self, i1=None, value=None, vary=True, vmin=-np.inf, vmax=np.inf, expr=None, brute_step=None, i2='all'):
    #     """add a parameter which is not a peak attr
    ...
    #     #######################
    #     if i1 is not None:
    #         if br.numanip.is_integer(i1): 
    #             i1 = int(i1)
    #     else: 
    #         raise ValueError(f'i1 must be an integer')

    #     ##############################
    #     # check i1, i2 pair is valid #
    #     ##############################
    #     if i2 == 'all':
    #         if i1 is not None:
    #             test = {f'i2:{i2}': i1 in self._get_additional_indexes_i1(i2=i2) for i2 in self._get_indexes_i2()}
    #             for key in test:
    #                 if test[key]:
    #                     raise ValueError(f'for i2=`all`, i1 must not exists for all i2\ni1_exists={test}')
    #         else:
    #             i1 = max([[max(self._get_additional_indexes_i1(i2=i2)) for i2 in self._get_indexes_i2()]]) + 1
    #     else:
    #         if br.numanip.is_integer(i2): 
    #             i2 = int(i2)
    #             if i1 in self._get_additional_indexes_i1(i2=i2):
    #                 raise ValueError(f'i1={i1} and i2={i2} already exist')
    #             else:
    #                 i1 = max(self._get_additional_indexes_i1(i2=i2)) + 1
    #         else: 
    #             raise ValueError(f'`i2 must be an integer or `all`')
    ...
    #             name = 'A_' + str(i1) + '_' + str(_i2)
    #             super().add(name, value=value, vary=vary, min=vmin, max=vmax, expr=expr, brute_step=brute_step)
    #     else:
    #         name = 'A_' + str(i1) + '_' + str(i2)
    #         super().add(name, value=value, vary=vary, min=vmin, max=vmax, expr=expr, brute_step=brute_step)

    # def remove_additional(self, i1, i2):
    #     raise NotImplementedError('not implemented')
    ...
    def _get_indexes_i1(self, i2=None):
        ...
        Does NOT includes additional parameters.
    ...
    def _get_indexes_i2(self):
        ...
        Includes additional A parameters.
    ...
    def _get_names_by_index(self, i1=None, i2=None):
        ...
        Returns:
            list
        """           
        i1, i2 = self._check_i1_i2(i1=i1, i2=i2, i1_req=False, i2_req=False)
    ...
    # basic #
    #########
    def append(self, amp=None, c=None, w=None, w1=None, w2=None, m=0, m1=0, m2=0, area=None, i1=None, i2='all'):
        """add a new peak
        ...
        Args:
            ...
            m1, m2
            area
            i1 (integer or None, optional): if None, i1 will be the smallest unique
                index i1 possible common for all i2. Default is None 
            i2 (number, str, optional): integer or `all`. 
        ...
        if i2 == 'all':
            if i1 is not None:
                test = {f'i2:{i2}': i1 in self._get_indexes_i1(i2=i2) for i2 in self._get_indexes_i2()}
                for key in test:
                    if test[key]:
                        raise ValueError(f'for i2=`all`, i1 must not exists for all i2\ni1_exists={test}')
            else:
                i1 = max([[max(self._get_indexes_i1(i2=i2)) for i2 in self._get_indexes_i2()]]) + 1
        else:
            if br.numanip.is_integer(i2): 
                i2 = int(i2)
                if i1 in self._get_indexes_i1(i2=i2):
                    ...
                    i1 = max(self._get_indexes_i1(i2=i2)) + 1
            else: 
                raise ValueError(f'`i2 must be an integer or `all`')
        ...
        if i2 == 'all':
            for _i2 in self._get_indexes_i2():
                self.append(i1=i1, i2=_i2, amp=amp, c=c, w=w, w1=w1, w2=w2, m=m, m1=m1, m2=m2, area=area)
            return
        else:
            if area is None:
                super().add(f'amp_{i1}_{i2}',  value=amp,  vary=True, min=-np.inf, max=np.inf, expr=None, brute_step=None)
            else:
                super().add(f'area_{i1}_{i2}', value=area, vary=True, min=-np.inf, max=np.inf, expr=None, brute_step=None)
            super().add(f'c_{i1}_{i2}',        value=c,    vary=True, min=-np.inf, max=np.inf, expr=None, brute_step=None)
            if w1 is not None and w2 is not None:
                super().add(f'w1_{i1}_{i2}', value=w1, vary=True,     min=0,       max=np.inf, expr=None, brute_step=None)
                super().add(f'w2_{i1}_{i2}', value=w2, vary=True,     min=0,       max=np.inf, expr=None, brute_step=None)
                super().add(f'm1_{i1}_{i2}', value=m1, vary=False,    min=0,       max=1,      expr=None, brute_step=None)
                super().add(f'm2_{i1}_{i2}', value=m1, vary=False,    min=0,       max=1,      expr=None, brute_step=None)
            else:
                super().add(f'w_{i1}_{i2}',  value=w,  vary=True,     min=0,       max=np.inf, expr=None, brute_step=None)
                super().add(f'm_{i1}_{i2}',  value=m,  vary=False,    min=0,       max=1,      expr=None, brute_step=None)
            return

    def remove(self, i1, i2='all'):
        """Remove peak.

        Args:
            i1 (int): peak index number to be removed.
            i2 (int or 'all', optional): secondary (spectrum) index i2. Default is 'all'.
    ...
    # get values #
    ##############
    def get_c(self, i1=None, i2=None):
        """Returns the center (position) of a peak

        i1 (int, optional): primary index i1. If i1='A', raises an error.
        i2 (int, optional): secondary (spectrum) index i2. Only used if 
            peaks has secondary indexes. Default is None.
        ...
        has_i2    = self._has_i2()
        unique_i2 = self._unique_i2()
        if has_i2 is None:
            return []
        elif has_i2 and unique_i2:
            i2 = self._get_indexes_i2()[0]
        elif has_i2 and unique_i2 == False:
            if i2 is None:
        ...
        #########
        if has_i2:
            return self[f'c_{i1}_{i2}'].value
        else:
            return self[f'c_{i1}'].value
    ...
    def plot(self, ax=None, offset=0, shift=0, factor=1, calib=1, **kwargs):
        ...
        if self._has_i2() and self._unique_i2() == False:
            i2_list      = self._get_indexes_i2()
            number_of_i2 = len(i2_list)
            final        = []
            ...
            for _i, i2 in enumerate(i2_list):
                for i1 in self._get_indexes_i1(i2=i2):
                    final += [self[i2][i1].plot(ax=ax, offset=offset[_i], shift=shift[_i], factor=factor[_i], calib=calib[_i], **kwargs), ]
            return final
        else:
            if self._has_i2():
                i2 = self._get_indexes_i2()[0]
            ...
            xerr = []
            for i1 in self._get_indexes_i1(i2=i2):
                c    += [self.get_c(i1=i1, i2=i2)]
                amp  += [self.get_amp(i1=i1, i2=i2)]
                xerr += [self.get_w(i1=i1, i2=i2)/2]
            ...
            final = ax.errorbar(c, amp, xerr=xerr, **kwargs)
